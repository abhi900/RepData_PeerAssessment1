...
The maximum number of steps on average are `r maxinterval$Int.Avg` and are found on the `r maxinterval$interval`.
...
```{r part3-updmeanandmedian}
dailymean_2<-mean(dailysum_2$Steps.sum)
dailymedian_2<-median(dailysum_2$Steps.sum)
```

The second dataset's daily step mean is `r dailymean_2` and the daily step median is `r dailymedian_2`. The daily step median is the same as in Part 1 (initial dataset), while the daily step mean of the second dataset has become equal with the daily step median of the two datasets.
